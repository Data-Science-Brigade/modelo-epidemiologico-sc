...
#
read_covid_data <- function(deaths_filename, population_filename, reference_date, 
                            start_pandemic = 30, allowed_locations = NULL) {
  require(readr)
  require(tidyverse)
  require(lubridate)
  ...
  cities_df <- cities_df %>%
    group_by(nom_regiaosaude, nom_regional, nom_municipio, data_ocorrencia) %>%
    summarise(casos = sum(casos), obitos = sum(obitos))


  #### COMPILE casos AND obitos FOR EACH CITY, REGION AND STATE ####
  healthregions_df <- cities_df %>%
    group_by(nom_regiaosaude, data_ocorrencia) %>%
    summarise(casos = sum(casos), obitos = sum(obitos))

  macrorregions_df <- cities_df %>%
    group_by(nom_regional, data_ocorrencia) %>%
    summarise(casos = sum(casos), obitos = sum(obitos))

  state_df <- macrorregions_df %>%
  ...
  healthregions_df <- healthregions_df %>%
    ungroup() %>%
    mutate(location_name = paste0("SC_RSA_", gsub(" ", "_", nom_regiaosaude))) %>%
    select(location_name, data_ocorrencia, casos, obitos)
  macrorregions_df <- macrorregions_df %>%
    ungroup() %>%
  ...
  df <- df %>%
    merge(all_dates_df, by = c("location_name", "data_ocorrencia"), all = T) %>%
    mutate(
...
read_pop <- function(population_filename) {
  ...
  SC_pop_macrorregions <- SC_pop %>%
    group_by(nom_regional) %>%
    summarise(pop = sum(qtd_populacao_estimada)) %>%
    ungroup() %>%
    mutate(location_name = paste0("SC_MAC_", gsub(" ", "_", nom_regional))) %>%
    select(location_name, pop)

  SC_pop_healthregions <- SC_pop %>%
    group_by(nom_regiaosaude) %>%
    summarise(pop = sum(qtd_populacao_estimada)) %>%
    ungroup() %>%
    mutate(location_name = paste0("SC_RSA_", gsub(" ", "_", nom_regiaosaude))) %>%
    select(location_name, pop)
