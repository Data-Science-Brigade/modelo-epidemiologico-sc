...
run_epidemiological_model <- function(stan_list,
  ...
  if (is.null(mode) && any(
    is.null(iter), is.null(warmup), is.null(chains),
  ...
  } else if (!is.null(mode) && any(
    !is.null(iter), !is.null(warmup),
  ...
  if (is.null(mode)) {
    ...
    if (is.null(iter)) {
      iter <- NAMED_MODELS$iter[[mode]]
    }
    if (is.null(warmup)) {
      warmup <- NAMED_MODELS$warmup[[mode]]
    }
    if (is.null(chains)) {
      chains <- NAMED_MODELS$chains[[mode]]
    }
    if (is.null(adapt_delta)) {
      adapt_delta <- NAMED_MODELS$adapt_delta[[mode]]
    }
    if (is.null(max_treedepth)) {
      max_treedepth <- NAMED_MODELS$max_treedepth[[mode]]
    }
    if (is.null(verbose)) {
      verbose <- NAMED_MODELS$verbose[[mode]]
    }
  }
  ...
  fit <- rstan::sampling(model,
    data = stan_list$stan_data, iter = iter, warmup = warmup, chains = chains, verbose = verbose,
