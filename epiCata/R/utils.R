...
get_merged_forecast_dfs <- function(location_names, model_output, forecast = 30, aggregate_name = NULL) {
  ...
  if (length(location_names) > 1) {
    ...
    missing_original_data <- data.frame(
      "time" = rr$min_missing_date + ifelse(rr$is_weekly, weeks, days)(0:(rr$fullN - 1)),
      "location_name" = rep(aggregate_name, rr$fullN),
      "reported_cases" = agg_reported_cases,
      "deaths" = agg_reported_deaths
    ) %>%
      mutate(cum_cases = cumsum(reported_cases)) %>%
      filter(cum_cases > 0, time < rr$min_date) %>%
      select(-c(cum_cases))
    ...
    data_location_forecast <- data.frame(
      "time" = rr$max_date + ifelse(rr$is_weekly, weeks, days)(0:rr$forecast),
