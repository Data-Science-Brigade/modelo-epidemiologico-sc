...
generate_multiwave_timeseries <- function(){
  # 1st variant
  # composed by 3 similar waves 
  g1_1 <- EnvStats::rgammaAlt(2000, mean=8, cv=0.12)
  hi1_1 <- hist(g1_1,breaks = seq(min(g1_1),max(g1_1), l = (length(g1_1)/8 + 1) ), plot=FALSE) 
  g1_2 <- EnvStats::rgammaAlt(2000, mean=8, cv=0.08)
  hi1_2 <- hist(g1_2,breaks = seq(min(g1_2),max(g1_2), l = (length(g1_2)/8 + 1) ), plot=FALSE) 
  g1_3 <- EnvStats::rgammaAlt(2000, mean=8, cv=0.10)
  hi1_3 <- hist(g1_3,breaks = seq(min(g1_3),max(g1_3), l = (length(g1_3)/8 + 1) ), plot=FALSE) 
  ...
  hi2_1 <- hist(g2_1,breaks = seq(min(g2_1),max(g2_1), l = (length(g2_1)/16 + 1) ), plot=FALSE)
  ...
  list("variant1"=deaths1, "variant2"=deaths2, "combined"=deaths_comb)
  
}



plot_combined_timeseries <- function(){
  
  comb_list <- generate_multiwave_timeseries()
  deaths1 <- comb_list$variant1
  deaths2 <- comb_list$variant2
  deaths_comb <- comb_list$combined
  ...
  # TODO: Take infection-to-death into consideration rather than the simple multiplication
