...
Como aqui estou usando o RNotebook, o bloco de código acima vai ser automaticamente executado e carregado no rstan. Isso vai gerar um objeto rstan chamado `simple_model`.

Caso esteja rodando o conteúdo na mão, salve o modelo stan em algum arquivo de texto, por exemplo `simple_model.stan` e então o leia para a variável `simple_model <- rstan::stan_model("simple_model.stan")`.

De qualquer forma, com o objeto `simple_model`, é possível fazer o sampling. Antes disso, vamos só configurar o stan para re-compilar o modelo sempre, por padrão, e para usar os múltiplos cores da máquina:
...
# Simulação 04: Adicionando dados de mobilidade

O objeto `stan_list` já contém dados previamente compilados do Google Mobility para a localidade informada.Esses dados ficam salvas na matriz multidimensional `stan_list$stan_data$X` (inclusive, `X` não é um bom nome para essa variável. Mantive pra ficar compatível com o código base original, mas poderia ser algo como `mobility` ou `covariates`).

Essa matriz tem 3 dimensões `(M, N2, P)`: localidade, número de dias modelados e tipo de mobilidade (`P`). Existem seis tipos de categorias de locais mapeados por esses dados [(fonte)](https://www.google.com/covid19/mobility/data_documentation.html?hl=en):
...
```{r eval=TRUE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
stan_list$stan_data$X[1,,] %>% head()
```

Outro local onde esses mesmos dados estão disponíveis, mas em outro formato, é no `stan_list$common_interventions`:
...
plot_df <- 
  stan_list$common_interventions %>% 
  pivot_longer(cols=-`DATA`, names_to="Mobility type") %>%
  mutate(`Mobility type`=gsub("_percent_change_from_baseline", "", `Mobility type`))

ggplot(plot_df, aes(x=DATA, y=value, color=`Mobility type`, group=`Mobility type`)) +
  geom_line() + geom_point() +
  ggthemes::theme_clean() + 
  geom_hline(yintercept = 0, linetype="dashed") +
  scale_x_date(name = "", labels = date_format("%e %b"), 
               limits=c(ymd("2020-03-05"), ymd("2021-06-05")),
               breaks=seq(ymd("2020-03-05"), ymd("2021-06-05"), by="3 months")) +
  scale_y_continuous(name="Change in mobility from pre-pandemic baseline", labels=scales::percent) + 
  ggthemes::scale_color_tableau()
```

Nós usaremos esses dados como covariáveis para o nosso modelo. A suposição aqui é a de que os padrões de mobilidade estão intimamente relacionados com a velocidade de propagação do vírus -- o Rt.

Adicionaremos uma variável `alpha` para atribuir um peso às variáveis de mobilidade, o que será usado depois para 

O modelo abaixo é o que a gente chama de modelo `base`:

```{stan eval=TRUE, output.var="base_model"}
data {
  int <lower=1> M; 
  int <lower=1> P; 
  int <lower=1> N0; 
  ...
  int deaths[N2, M];
  matrix[N2, M] f;
  matrix[N2, P] X[M]; // dados de variação percentual da mobilidade
  real pop[M];
  real SI[N2];
}
...
Vamos rodar uma nova simulação com esse modelo:

```{r eval=TRUE, message=FALSE, warning=FALSE}
require(rstan)

fit <- 
  rstan::sampling(base_model,
                  data = stan_list$stan_data, iter = 1000, warmup = 500, 
                  chains = 1, verbose = TRUE,
                  control = list(adapt_delta = 0.95, max_treedepth = 7))
```
...
Zero divergências! Isso é ótimo.

```{r eval=TRUE, message=TRUE, warning=FALSE, fig.width = 8, fig.height=5}
rstan::check_hmc_diagnostics(model_output$fit)
```
...
fit_df <- as.data.frame(model_output$fit)
cols <- fit_df %>% select(tau, y, kappa, contains("mu")) %>% colnames()
...
fit_df <- as.data.frame(model_output$fit)
cols <- fit_df %>% select(contains("alpha")) %>% colnames()
...
Dessa vez o R0 ficou próximo de 3 e agora ele oscila bastante com o tempo, de acordo com as variações do Google Mobility:
...
A curva epidêmica estimada consegue acompanhar relativamente bem as três ondas. Porém, note que, para o modelo, a  "explicação" para o aumento inesperado nos óbitos da terceira onda se deve a um número bem maior de infectados do que o número reportado:
...
Na prática, o mais provável é que essa terceira onda represente na verdade a propagação da variante P.1 (https://science.sciencemag.org/content/372/6544/815). Como não há nenhuma variável indicando ao modelo sobre mudanças nas dinâmicas de transmissão inerentes ao vírus, a explicação plausível que resta é aumentar o número de casos.

A estimativa para a curva de óbitos ficou bem coerente desta vez:
